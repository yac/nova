...
def ec2_error_ex(ex, req, code=None, message=None, unexpected=False):
    ...
    Unexpected 5xx errors may contain sensitive data,
    supress their messages for security.
    """
    import IPython
    IPython.embed()
    if not code:
        code = faults.exception_to_ec2code(ex)
    status = getattr(ex, 'code', 500)

    if unexpected:
        log_msg = "Unexpected %(ex_name)s raised"
        if status >= 500:
            log_fun = LOG.critical
        else:
            log_fun = LOG.error
    else:
        log_msg = "%(ex_name)s raised"
        log_fun = LOG.debug

    if ex.args:
        if not message and (not unexpected or status < 500):
    ...
    return faults.ec2_error_response(request_id, code, message,
        status=status)
...
class Executor(wsgi.Application):
    ...
    def __call__(self, req):
        ...
        try:
            ...
            return ec2_error_ex(ex, req, code=code, message=message)
            ...
            return ec2_error_ex(ex, req, code=code)
                ...
                exception.FloatingIpLimitExceeded) as ex:
            code = 'ResourceLimitExceeded'
            return ec2_error_ex(ex, req, code=code)
        except exception.VolumeUnattached as ex:
            code = 'IncorrectState'
            return ec2_error_ex(ex, req, code=code)
        except exception.InvalidInstanceIDMalformedEC2 as ex:
            message = _('Invalid id: bogus (expecting "i-...")')
            return ec2_error_ex(ex, req, message=message)
        except (exception.InvalidParameterValue,
                exception.InvalidPortRange,
                exception.MissingParameterEC2,
                exception.InvalidPermissionDuplicateEC2,
                exception.InternalErrorEC2,
                # NOTE(jruzicka): Following aren't valid EC2 errors.
